...
export default class ActiveTimes extends React.Component {
	...
	async loadData() {
		...
		const analyticsLink = course.Links.filter(
			x => x.rel === ANALYTICS_LINK
		)[0];
		const results = (await service.get(analyticsLink.href)) || {};
		const activeTimesSummaryLink = (results.Links || []).filter(
			x => x.rel === ACTIVE_TIMES_SUMMARY_LINK
		)[0];
		const summaryData =
			(await service.get(activeTimesSummaryLink.href)) || {};
