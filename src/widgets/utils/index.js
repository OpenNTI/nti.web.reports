...
export function determineBlockColor(
	value,
	minValue = 0,
	maxValue,
	customColors
...
function processData(rawData, rangeStart, rangeEnd) {
	...
	for (let i = start; i <= rangeEnd; i += INCREMENT) {
		...
		const isLastWeekADifferentMonth =
			(currLatestForDay && !currLatestForDay.date) ||
			currLatestForDay.date.getMonth() !== curr.getMonth();

		const isNextWeekADifferentMonth =
			nextWeek.getMonth() !== curr.getMonth();
...
function oneYearBeforeNow() {
	...
	return new Date(
		now.getMonth() + 1 + '/' + now.getDate() + '/' + (now.getFullYear() - 1)
	);
}
...
export async function loadDailyActivity(entity, startDate, endDate) {
	...
	try {
		if (entity) {
			const analyticsLink = entity.Links.filter(
				x => x.rel === ANALYTICS_LINK
			)[0];
			const results = (await service.get(analyticsLink.href)) || {};
			const activityByDateSummary = (results.Links || []).filter(
				x => x.rel === ACTIVITY_BY_DATE_SUMMARY_LINK
			)[0];
			summaryData =
				(await service.get(activityByDateSummary.href + paramStr)) ||
				{};
		} else {
			// default to global if no entity specified
			const analyticsWorkspace = service.getWorkspace('Analytics');
			const activityByDateSummary = (
				analyticsWorkspace.Links || []
			).filter(x => x.rel === ACTIVITY_BY_DATE_SUMMARY_LINK)[0];
			summaryData =
				(await service.get(activityByDateSummary.href + paramStr)) ||
				{};
		}
