...
const t = scoped(
	'web-reports.context.course-instance.assignments.Item',
	DEFAULT_TEXT
);
...
export default class ReportCourseInstanceAssignmentItem extends React.Component {
	...
	render() {
		...
		return (
			<div
				className={cx('report-course-instance-assignment-item', {
					disabled,
				})}
			...
			>
				...
				{due ? (
					<div className="date">
						<span>{t('due')}</span>
						<DateTime date={due} format={dateFormat} />
					</div>
				) : start ? (
					<div className="date">
						<span>{t('available')}</span>
						<DateTime date={start} format={dateFormat} />
					</div>
				) : (
